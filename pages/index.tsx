...
const Home: React.FC = () => {
  ...
  const toggleColorScheme = () => {
    setColorScheme((prevScheme) => {
      if (prevScheme === "green") return "berry";
      if (prevScheme === "berry") return "blue";
      return "green"; // default back to green
    });
  };
  ...
  return (
    <div className={`bg-c-${colorScheme}-medium relative`}>
      <button
        onClick={toggleColorScheme}
        className="p-6 bg-yellow-600 absolute top-0 left-0 z-50"
      >
        Switch to{" "}
        {colorScheme === "green"
          ? "blue"
      ...
      <main>
        ...
        <button
          onClick={scrollToTop}
          className={`fixed bottom-5 right-5 p-2  rounded-full bg-c-${colorScheme}-dark text-c-${colorScheme}-gray cursor-pointer focus:outline-none  transition-all`}
